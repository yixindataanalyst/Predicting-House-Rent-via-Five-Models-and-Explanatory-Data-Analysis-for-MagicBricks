...
my_palette <- c("#4D4D4D", "#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442")

ggplot(data, aes(x=City, y=Rent, fill=City)) +
  geom_bar(stat="summary", fun="mean") +
  coord_flip() +
  theme_grey() +
  scale_fill_manual(values=my_palette, name="City") +
  labs(title="What city has higher rent?", x="City in India", y="Rent") +
  geom_hline(yintercept=mean(data$Rent), linewidth=1, color="blue")


ggplot(data = data, aes(x=City,y=Rent,color=City)) + 
  geom_boxplot()+
  geom_jitter(shape=16, position=position_jitter(0.2))+
  labs(title = 'Data point distribution of different cities',
       y='Rent',x='City in India')


## EDA: Plotting numerical data
library(funModeling)
plot_num(data, bins = 10, path_out = NA)

##EDA: Plotting categorical data
library(inspectdf)
x <- inspect_cat(df1 =data, df2 = NULL, include_int = FALSE)
x %>% show_plot()


## Listing and Plotting outliers
library("dlookr")
dlookr::diagnose_outlier(data)
dlookr::plot_outlier(data)

## Correlation visualization
library("PerformanceAnalytics")
chart.Correlation(data[,c(2:4,10,30:31)], histogram = TRUE, pch = 15)
...
### matrix the variables (one-hot encoding )
...
Xgboost.OOS<-data.frame(Xgboost=rep(NA,nfold)) 
library(xgboost)
...
for(k in 1:nfold){ 
  ...
  Xgb<-xgboost(data = as.matrix(data_Lasso[train,-1]), label = data_Lasso$Rent[train], nrounds = 10,eval.metric = "error",objective = "reg:linear")
  ### Get the out-of-sample prediction for XGBoost
  predXgb = predict(Xgb, newdata = as.matrix(data_Lasso[-train,][-1]))
  ### Calculate the R square for out-of-sample XGBoost forecasting. 
  Xgboost.OOS$Xgboost[k] <- 1 - (sum((data_Lasso$Rent[-train]-predXgb)^2)/sum((data_Lasso$Rent[-train]-mean(data_Lasso$Rent[-train]))^2))
...
R2performance <- cbind(PL.OOS,L.OOS,OOS,RandomForest.OOS,Xgboost.OOS)
...
##The XGBoost preforms way more better than other models with an average
##R2 of 0.00
